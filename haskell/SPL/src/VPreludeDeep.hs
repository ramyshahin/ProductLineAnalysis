...
map' _f xs  = let case0 __cntxt__ = (mkVar [] __cntxt__)
...
filter' _p xs  = let case0 __cntxt__ = (mkVar [] __cntxt__)
                     split0 __dummy__ = case __dummy__ of [] -> ()
                 ...
                 in  --assert (not (isNilVar xs)) $
                     assert (disjInv xs) $
                     liftedCase (xs) (\__dummy__ -> case __dummy__ of [] -> 0
                                                                      (y : ys) -> 1) 
                                     [\__cntxt__ -> (uncurry0 (case0 __cntxt__)) . (liftV split0), 
                                      \__cntxt__ -> (uncurry2 (case1 __cntxt__)) . (liftV split1)]
