...
If we are to rewrite FLDPL using CPP macros, it would look like listing 
ref{lst:Annotative}. A single feature (e.g. \emph{LOW\_GAS\_WARNING} is now scattered across the code base, and in some cases replicated.
...
\begin{lstlisting}[caption=Delta-Oriented example, label={lst:Delta}]
core FLDGas {
	class FLD {
		void reportFuelLevel(float level) { 
			dashboard.display(level); 
